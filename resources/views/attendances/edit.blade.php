...
<div class="card p-4" style="max-width: 600px;">
    ...
    <form action="{{ route('attendances.update', $attendance->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="date" name="attendance_date" value="{{ old('attendance_date', $attendance->attendance_date) }}" class="form-control @error('attendance_date') is-invalid @enderror" required>
        ...
        <div class="mb-3">
            ...
            <select name="status" class="form-select @error('status') is-invalid @enderror">
                @foreach (['Present','Absent','Late','Excused'] as $status)
                    <option value="{{ $status }}" {{ old('status', $attendance->status) == $status ? 'selected' : '' }}>{{ $status }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <textarea name="remark" class="form-control @error('remark') is-invalid @enderror" rows="2">{{ old('remark', $attendance->remark) }}</textarea>
